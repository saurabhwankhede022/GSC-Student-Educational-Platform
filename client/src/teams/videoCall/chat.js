...
import {
    IconButton, Button, TextField, Dialog, DialogActions, DialogContent, DialogContentText,
    DialogTitle, List, ListItem, Typography, Divider, Tooltip
...
const Chat = () => {
    ...
    const exportChat = () => {
        ...
        doc.setFontSize(10); // Set font size for chat content

        chats.forEach((chat) => {
            doc.text(i, j, chat.senderEmail.substring(0, chat.senderEmail.indexOf('@')));
            doc.text(i + 110, j, '-');
            doc.text(i + 115, j, chat.message);
            j = j + 20; // Increment y-coordinate for the next line
        });
